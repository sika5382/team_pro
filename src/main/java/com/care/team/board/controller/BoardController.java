...
public class BoardController {
	...
	@Autowired private BoarderService bs;
	@Autowired private BoardFileService bfs;
	...
	@RequestMapping(value="boardLocalList", method= {RequestMethod.GET, RequestMethod.POST})
	public String selectLocalBoardList(Model model, @RequestParam("local") String local,
			@RequestParam(value="num", required=false, defaultValue = "1") int num) {
		System.out.println("local:"+local);
		bs.selectLocalBoardList(model, local, num);
		return "board/boardLocalList";
	}
